...
```
The following script, placed on the prefeb of the generated ball, can detect whether there is a player within the trigger range and call the checkPickup method in the PickUp script
```c#
...
```
The following script, placed on the Player controller, takes player input and calls built-in methods to pick up and throw the ball
```c#
...
```
The following script is placed on the trigger in the hole to detect whether a ball enters, and if so, call the method in the Point script to change the text
```c#
...
```
The following script, placed on the text UI that needs to be changed, gets the text and changes it
```c#
